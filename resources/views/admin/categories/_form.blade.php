                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="parent_id" class="form-select form-select-sm">
                            ...
                            @php
                                $parentCats = $categories->whereNull('parent_id');
                            @endphp
                            @foreach ($parentCats as $pc)
                                @if (! in_array($pc->id, $excludeIds))
                                    <option value="{{ $pc->id }}" {{ old('parent_id', $category->parent_id ?? '') == $pc->id ? 'selected' : '' }}>{{ $pc->name }}</option>
                                    @php
                                        $childCats = $categories->where('parent_id', $pc->id);
                                    @endphp
                                    @foreach ($childCats as $cc)
                                        @if (! in_array($cc->id, $excludeIds))
                                            <option value="{{ $cc->id }}" {{ old('parent_id', $category->parent_id ?? '') == $cc->id ? 'selected' : '' }}>&nbsp;&nbsp;— {{ $cc->name }}</option>
                                        @endif
                                    @endforeach
                                @endif
                            @endforeach
...
<div class="d-flex gap-2 mt-3">
    <button type="submit" class="btn btn-sm btn-primary"><i class="bi bi-check-lg me-1"></i>Save Category</button>
    <a href="{{ route('admin.categories.index') }}" class="btn btn-sm btn-outline-secondary">Cancel</a>
</div>
